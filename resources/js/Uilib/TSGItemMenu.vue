...
const emit = defineEmits(['deleteObject','setActiveObject','tsComCallback']);
...
<ul class="menu w-full rounded-box gap-2 ">
  <li v-for="(item, index) in props.init" :key="'TSGM_'+index">
    <a class="w-full flex justify-between" :class=" {'dark:bg-gray-700':currentIndex == index}">

      <!-- 隐藏显示对象 -->
      <svg v-if="item.get('visible')" @click="emit('tsComCallback','visible',false,index)" class="w-[18px] h-[18px] mr-1 dark:hover:fill-slate-50" fill="currentColor" viewBox="0 0 576 512" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- 锁定对象 -->
      <svg v-if="item.get('selectable')" @click="emit('tsComCallback','selectable',false,index)" class="mr-1 w-[18px] h-[18px] dark:hover:fill-slate-50" fill="currentColor" viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="flex gap-1 justify-start items-center w-full overflow-clip cursor-pointer"  @click="setActiveObject(index)">
        <div v-if="item.type == 'image'">
          <img class="w-8 h-8" :src="item.getSrc()"/>
        </div>
        <div v-else>
          {{item.text}}
        </div>
      ...
      <!-- 删除对象 -->
      <svg @click="deleteObject(index)" class="dark:hover:fill-slate-50 w-[18px] h-[18px]" fill="currentColor"  viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
