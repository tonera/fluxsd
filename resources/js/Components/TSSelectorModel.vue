...
const page = usePage();
//base :base model v=1.5/sdxl
const props = defineProps(['name','init','label','def','engine','base']);
const emit = defineEmits(['tsComCallback']);
const show_more = ref(false);
const cItem = ref( {'name':props.name,'val':props.def??'','thumb':null});
...
var initOptions = ['1.5','pre1.5','sdxl','presdxl','presd3'];
//selected options
...
watch(() =>props.engine, (e)=>{
  if(props.def == null){
    cItem.value = {'name':props.name,'val':props.def??'','thumb':null};
  }
  getModelList( 1 , props.init);
  cPage.value = 1;
});
...
watch(() =>props.base, (e)=>{
  ...
  initOptions = [];
  if(props.base == '1.5'){
    options.push(...['1.5','pre1.5']);   
    initOptions.push(...['1.5','pre1.5']); 
  }else{
    options.push(...['sdxl','presdxl','presd3']);   
    initOptions.push(...['sdxl','presdxl','presd3']); 
  }
  getModelList(1, props.init );
});
watch(() =>props.def, (e)=>{
...
async function getModelList(page , type){
  let res = await aiHttpRequest('get', '/api/models', {
    page:page, engine:props.engine, types:JSON.stringify([type]), options:JSON.stringify(options),service:1
  });
  if(res.code == 1){
    models.value = res.data;
    total.value = res.meta.last_page;
    initOptions = res.base_models;
  }
  // console.log(res);
}
...
async function handleFavored(item, index){
  ...
  if(res.code == 1){
    ...
    showTopSnackbar(trans('Operation failed'), 'error');
  }
}
...
function setValue(item){
  // console.log(item);
  if(props.engine == 'lc' && item.is_download != 1){
    showTopSnackbar('You must download this model before using it', 'error');
    return;
  }
    try{
        show_more.value = false;
        cItem.value.val = item.sd_name;
        cItem.value.thumb = item.thumb;
...
<div class="join py-4">
    ...
    <div class="relative join-item">
      <div class="h-12 bg-white flex border border-gray-200 rounded items-center">
        <img v-show="cItem.thumb!=undefined" class="object-cover w-12 h-12 pr-1" :src="cItem.thumb"/>
      ...
      <div 
        ...
        <!-- sync model -->
        <div class="flex justify-end pt-2 pr-4 gap-x-2 items-center">
          <a class="text-xs" href="/models" target="_blank">{{trans('market')}}>></a>
          <button class="btn btn-outline btn-sm" @click="show_more = false">{{trans('close')}}</button>
        </div>
        <!-- options -->
        <div class="p-2 flex justify-between h-16">
          ...
          <ul class="menu lg:menu-horizontal z-30 w-40 justify-end">
            <li>
              <details close>
                <summary class="w-40 justify-end">{{trans('filter')}}</summary>
                <ul>
            ...
            <div 
                ...
                <div @click="setValue(item)" class="relative h-60">
                  <div class="absolute z-20 inset-0 bg-gray-900 opacity-0 transition-opacity duration-300 hover:opacity-80 flex items-center justify-center text-white whitespace-pre-wrap overflow-wrap-break-word break-all p-2">
                    {{item.sd_name}}:{{item.base_model}}
                  </div>
                  <div class="absolute inset-0">
                      <img class="object-cover h-full w-full" :src="item.thumb"/>
                  </div>
                </div>
